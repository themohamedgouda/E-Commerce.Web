...
namespace ServicesImplementationLayer
{
    public class ServicesManager(IUnitOfWork _unitOfWork, IMapper _mapper, IBasketRepository _basketRepository , UserManager<ApplicationUser> _userManager , IConfiguration _configuration) : IServicesManager
    {
        ...
        private readonly Lazy<IAuthenticationServices> _LazyAuthenticationServices = new Lazy<IAuthenticationServices>(() => new AuthenticationServices(_userManager, _configuration , _mapper));
        private readonly Lazy<IOrderServices> _LazyOrderServices = new Lazy<IOrderServices>(() => new OrderServices(_mapper, _basketRepository, _unitOfWork));
